 ...
 *
 * The workflow itself is the Position Monitor's job (Lane A, T-A7). What is
 * enforced here is the second half of the sentence: while a position is past
 * the point where it should have been closed, the agent may not open anything
 * new. An agent that keeps entering while it cannot exit is exactly the
 * failure mode the mandate exists to prevent.
 *
...
export const covenant07: InvariantEvaluator = {
  ...
  evaluate({ account }) {
    const overdue = account.openPositions.filter((position) => position.dte <= 0);
